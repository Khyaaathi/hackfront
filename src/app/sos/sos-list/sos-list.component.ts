import { Component, ElementRef, ViewChild } from '@angular/core';
import { FormGroup, UntypedFormBuilder } from '@angular/forms';
import { Base64 } from 'js-base64';
import { ToastrService } from 'ngx-toastr';
import { ApiService } from 'src/app/core/services/api.service';
import { formatDate } from 'src/app/core/util/format-date-util';

@Component({
  selector: 'app-sos-list',
  templateUrl: './sos-list.component.html',
  styleUrls: ['./sos-list.component.css'],
})
export class SosListComponent {
  sosData: any;
  dataCount: number;
  loading: boolean;
  filterItemForm: FormGroup;
  filteredItems: any = [];
  filterFormData: any;
  grievanceStatus: string = 'Open';
  policeComment: string;
  currentIndex = 0;
  currentGrievance: any;
  page = 1;
  currentDate = formatDate(new Date());
  yesterdayDate = formatDate(
    new Date(new Date().setDate(new Date().getDate() - 1))
  );
  beforeYesterdayDate = formatDate(
    new Date(new Date().setDate(new Date().getDate() - 2))
  );
  filterDate = this.currentDate;
  @ViewChild('modalButton') ModalTrigger: ElementRef;
  constructor(
    private apiService: ApiService,
    private toastrService: ToastrService,
    private fb: UntypedFormBuilder
  ) {}
  ngOnInit() {
    this.getData(1);
    this.filterItemForm = this.fb.group({
      categoryName: [''],
      status: [''],
      fromDate: [''],
      toDate: [''],
      token: [''],
      phoneNumber: [''],
    });
  }
  dateSelect(date: any) {
    this.filterDate = date;
    this.getData(1);
  }
  changeStatus(val: any, item: any, index: number) {
    this.currentIndex = index;
    this.currentGrievance = item;
    this.grievanceStatus = val.value;
    this.ModalTrigger.nativeElement.click();
  }
  cancel() {
    this.sosData[this.currentIndex].status = 'Open';
  }
  refreshData() {
    this.getData(this.page);
  }
  saveStatus() {
    if (!this.policeComment) {
      this.sosData[this.currentIndex].status = 'Open';
      return;
    }
    const data = {
      id: this.currentGrievance.grievanceId,
      data: {
        status: this.grievanceStatus,
        comments: this.policeComment,
      },
    };
    this.loading = true;
    this.apiService
      .put('updateGrievanceById/', {
        data: Base64.encode(JSON.stringify(data)),
      })
      .subscribe(
        (data) => {
          this.policeComment = '';
          this.toastrService.success(
            $localize`:@@common-success-statusmsg:Status Updated`,
            $localize`:@@common-success-statustitle:Success`
          );
          this.loading = false;
        },
        (err) => {
          this.policeComment = '';
          this.sosData[this.currentIndex].status = 'Open';
          this.toastrService.error(
            $localize`:@@common-err-msg:Something Went Wrong`,
            $localize`:@@common-err-title:Server Error`
          );
          this.loading = false;
        }
      );
  }
  PageChange(event: any) {
    this.page = event.pageIndex + 1;
    if (
      !this.filterFormData?.categoryName &&
      !this.filterFormData?.status &&
      !this.filterFormData?.fromDate &&
      !this.filterFormData?.toDate &&
      !this.filterFormData?.token &&
      !this.filterFormData?.phoneNumber
    ) {
      this.getData(event.pageIndex + 1);
      return;
    }
    this.filterData(event.pageIndex + 1);
  }
  getData(page: number) {
    this.loading = true;
    this.apiService
      .get('getGrievance/' + page + '/' + this.filterDate)
      .subscribe(
        (data) => {
          this.sosData = data.data;
          this.dataCount = data.count;
          this.loading = false;
        },
        (err) => {
          this.toastrService.error(
            $localize`:@@common-err-msg:Something Went Wrong`,
            $localize`:@@common-err-title:Server Error`
          );
          this.loading = false;
        }
      );
  }
  filterData(page: number) {
    const currentDate = new Date(new Date().toString().split('GMT')[0] + ' UTC')
      .toISOString()
      .split(':');
    this.filterFormData = {
      categoryName: this.filterItemForm.controls['categoryName'].value,
      status: this.filterItemForm.controls['status'].value,
      fromDate: this.filterItemForm.controls['fromDate'].value
        ? this.filterItemForm.controls['fromDate'].value
        : '2023-05-01T00:00',
      toDate: this.filterItemForm.controls['toDate'].value
        ? this.filterItemForm.controls['toDate'].value
        : currentDate[0] + ':' + currentDate[1],
      token: this.filterItemForm.controls['token'].value,
      phoneNumber: this.filterItemForm.controls['phoneNumber'].value,
    };
    if (
      !this.filterFormData.categoryName &&
      !this.filterFormData.status &&
      !this.filterFormData.fromDate &&
      !this.filterFormData.toDate &&
      !this.filterFormData.token &&
      !this.filterFormData.phoneNumber
    ) {
      this.getData(1);
      return;
    }
    this.loading = true;
    this.apiService
      .post('/filterGrievanceItem/' + page + '/', {
        data: Base64.encode(JSON.stringify(this.filterFormData)),
      })
      .subscribe(
        (data) => {
          this.sosData = data.data;
          this.dataCount = data.count;
          this.loading = false;
        },
        (err) => {
          this.toastrService.error(
            $localize`:@@common-err-msg:Something Went Wrong`,
            $localize`:@@common-err-title:Server Error`
          );
          this.loading = false;
        }
      );
  }
}
